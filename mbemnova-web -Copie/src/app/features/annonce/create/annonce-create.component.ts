...
/* ─── MOCK ──────────────────────────────────────────────────────────────────── */
/*
interface LocalisationResponse { id: number; ville: string; quartier: string; active: boolean; }
interface TypeBienResponse     { id: number; nom: string; icone?: string; active: boolean; }
interface AnnonceDashboardResponse { id: number; [k: string]: any; }

const MOCK_TYPES: TypeBienResponse[] = [
  { id: 1, nom: 'Appartement', active: true },
  { id: 2, nom: 'Studio',      active: true },
  { id: 3, nom: 'Villa',       active: true },
  { id: 4, nom: 'Maison',      active: true },
  { id: 5, nom: 'Bureau',      active: true },
  { id: 6, nom: 'Boutique',    active: true },
  { id: 7, nom: 'Chambre',     active: true },
  { id: 8, nom: 'Terrain',     active: true },
];

const MOCK_VILLES = ['Douala', 'Yaoundé', 'Bafoussam', 'Garoua', 'Bamenda'];

const MOCK_QUARTIERS: Record<string, LocalisationResponse[]> = {
  'Douala':    [
    { id: 1, ville: 'Douala', quartier: 'Akwa',     active: true },
    { id: 2, ville: 'Douala', quartier: 'Bonanjo',  active: true },
    { id: 3, ville: 'Douala', quartier: 'Bonabéri', active: true },
    { id: 4, ville: 'Douala', quartier: 'Deïdo',    active: true },
    { id: 5, ville: 'Douala', quartier: 'New Bell', active: true },
    { id: 6, ville: 'Douala', quartier: 'Bali',     active: true },
    { id: 7, ville: 'Douala', quartier: 'Ndokotti', active: true },
    { id: 8, ville: 'Douala', quartier: 'Logpom',   active: true },
  ],
  'Yaoundé':  [
    { id: 10, ville: 'Yaoundé', quartier: 'Bastos',               active: true },
    { id: 11, ville: 'Yaoundé', quartier: 'Centre administratif', active: true },
    { id: 12, ville: 'Yaoundé', quartier: 'Mvan',                 active: true },
    { id: 13, ville: 'Yaoundé', quartier: 'Biyem-Assi',           active: true },
    { id: 14, ville: 'Yaoundé', quartier: 'Nkongsamba',           active: true },
  ],
  'Bafoussam': [
    { id: 20, ville: 'Bafoussam', quartier: 'Banengo', active: true },
    { id: 21, ville: 'Bafoussam', quartier: 'Kamkop',  active: true },
  ],
  'Garoua': [
    { id: 30, ville: 'Garoua', quartier: 'Plateau',  active: true },
    { id: 31, ville: 'Garoua', quartier: 'Bibémiré', active: true },
  ],
  'Bamenda': [
    { id: 40, ville: 'Bamenda', quartier: 'Commercial Avenue', active: true },
    { id: 41, ville: 'Bamenda', quartier: 'Up Station',        active: true },
  ],
};

class MockTypeBienApi {
  getAll() { return of({ data: MOCK_TYPES }); }
}
class MockLocalisationApi {
  getVilles()             { return of({ data: MOCK_VILLES }); }
  getQuartiers(v: string) { return of({ data: MOCK_QUARTIERS[v] ?? [] }); }
  creerQuartier(body: { ville: string; quartier: string }) {
    const nq: LocalisationResponse = { id: Date.now(), ville: body.ville, quartier: body.quartier, active: true };
    if (!MOCK_QUARTIERS[body.ville]) MOCK_QUARTIERS[body.ville] = [];
    MOCK_QUARTIERS[body.ville].push(nq);
    return of({ data: nq });
  }
}
class MockAnnonceApi {
  publier(_: any) { return of({ data: { id: Math.floor(Math.random() * 9000) + 1000 } as AnnonceDashboardResponse }); }
}
class MockPhotoApi {
  uploadPhotos(_id: number, _files: File[]) { return of(null); }
}
class MockStorageService {
  private key = 'draft_annonce_v3';
  saveDraft(v: any)       { localStorage.setItem(this.key, JSON.stringify(v)); }
  getDraft<T>(): T | null { try { return JSON.parse(localStorage.getItem(this.key) ?? 'null'); } catch { return null; } }
  hasDraft(): boolean     { return !!localStorage.getItem(this.key); }
...
class MockToastService {
  ...
  info(msg: string)    { console.info('[Toast ℹ️]', msg); }
}
*/
/* ─── FIN MOCK ────────────────────────────────────────────────────────────── */
...
@Component({
  ...
  styles: [
    `
      ...
      .step-bubble.done svg,
      .step-bubble.active svg {
        width: 15px;
        height: 15px;
      }
      .step-lbl {
      ...
      .type-tile.sel .type-tile-icon {
        background: #1e3a5f;
      }
      .type-tile.sel .type-tile-icon svg {
        color: #fff;
      }
      .type-tile-name {
        font-size: 10px;
      ...
      /* Input standard — même design que login */
      .input {
        width: 100%;
        height: 50px;
        padding: 0 16px;
  ...
  template: `
    <div class="page-bg">
      ...
      <header class="top-bar">
        ...
        <span class="top-bar-center">Publier une annonce</span>
        <div class="top-bar-side"></div>
      </header>

      <!-- ── Stepper ── -->
      <nav class="stepper-wrap" aria-label="Étapes du formulaire">
        ...
        @if (published()) {
          ...
          <div class="card">
            <div class="success-wrap fade-in">
              ...
              <p class="success-title">Annonce publiée !</p>
              <p class="success-sub">Votre annonce est en ligne et visible pendant 30 jours.</p>
          ...
          <div class="card">
            ...
            @if (hasDraft()) {
              <div class="draft-banner">
                ...
                <button type="button" class="draft-clear" (click)="clearDraft()">Effacer</button>
              </div>
            }
            ...
            <form [formGroup]="form" (ngSubmit)="onSubmit()" novalidate>
              ...
              @if (currentStep() === 1) {
                <div class="fade-in">
                  <div class="step-header">
                    ...
                    <p class="step-sub">Quel type de logement proposez-vous ?</p>
                  </div>
              ...
              @if (currentStep() === 2) {
                <div class="fade-in">
                  <div class="step-header">
                    ...
                    <p class="step-sub">Décrivez votre bien et fixez le loyer</p>
                  </div>
                  ...
                  <div class="section">
                    ...
                    <div class="field">
                      <div class="textarea-wrap">
                        ...
                        <span class="char-count" [class.warn]="descLen() > 900">
                          {{ descLen() }}/1000
                        </span>
                      </div>
                      @if (descLen() > 0 && descLen() < 30) {
                        <div class="field-error">
                          ...
                          Encore {{ 30 - descLen() }} caractère(s) requis (min. 30)
                        </div>
                      } @else {
                  ...
                  <div class="section" style="margin-top:20px">
                    ...
                    <div class="field">
                      ...
                      <div class="input-wrap">
                        ...
                        <input
                          type="number"
                          class="input has-icon-l has-sfx"
                          formControlName="prix"
                          min="1000"
                          [class.is-error]="isFieldError('prix')"
                          [class.is-valid]="
                            form.value.prix !== null && (form.value.prix ?? 0) >= 1000
                          "
                      ...
                      <div class="input-wrap">
                        <span class="wa-prefix">
                          <svg
                            viewBox="0 0 24 24"
                            fill="currentColor"
                            style="color:#25D366;width:15px;height:15px"
                          >
                            <path
              ...
              <!-- ══════════ ÉTAPE 3 — Photos + récap + publication ══════════ -->
              @if (currentStep() === 3) {
                <div class="fade-in">
                  <div class="step-header">
                    <p class="step-title">Photos & publication</p>
                    <p class="step-sub">Ajoutez des photos pour plus de contacts</p>
                  </div>
                  ...
                  <div class="section">
                    <div class="section-label">
                      ...
                      Photos
                      <span
                        style="font-weight:400;color:#9ca3af;text-transform:none;letter-spacing:0"
                        >(facultatif)</span
                      >
                    </div>
                    ...
                    <div class="photo-meta">
                      <span class="photo-tip"
                        >Les annonces avec photos reçoivent 5× plus de contacts</span
                      >
                    ...
                    <div class="recap-box">
                      ...
                      <div class="recap-row">
                        ...
                        <div>
                          ...
                          <div class="recap-val">
                            @if (photos().length === 0) {
                              Aucune — vous pourrez en ajouter plus tard
                            } @else {
...
export class AnnonceCreateComponent implements OnInit, OnDestroy {
  ...
  /* Mocks désactivés */
  // private readonly locApi      = new MockLocalisationApi();
  // private readonly typeBienApi = new MockTypeBienApi();
  // private readonly annonceApi  = new MockAnnonceApi();
  // private readonly photoApi    = new MockPhotoApi();
  // private readonly storage     = new MockStorageService();
  // private readonly toast       = new MockToastService();
  ...
  readonly form = this.fb.group({
    ...
    ville: [''],
    // localisationId sert uniquement à catégoriser la zone côté backend si nécessaire,
    // mais `quartier` est un champ texte libre (pas un id de quartier).
  ...
  readonly canAddQuartier = computed(() => false); // plus besoin
...
readonly step2Invalid = computed(() => {
  // Lire les valeurs via getRawValue() ne fonctionne pas dans computed
  // Il faut écouter les valueChanges — utiliser un signal dédié
  const v = this.formValue(); // ← signal qui suit les changements
  return (
    !v.description ||
    (v.description?.length ?? 0) < 30 ||
    !v.prix ||
    v.prix < 1000 ||
    !v.whatsappRaw ||
...
ngOnInit(): void {
  ...
  // ✅ Charger les villes D'ABORD, puis appliquer le draft
  this.locApi.getVillesAvecId().subscribe(r => {
    this.villesAvecId.set(r.data ?? []);

    // Appliquer le draft APRÈS que les villes sont disponibles
    const draft = this.storage.getDraft<any>();
    if (draft) {
      this.form.patchValue(draft);

      // Recalculer localisationId depuis la ville du draft
      if (draft.ville) {
        const found = this.villesAvecId().find(v => v.ville === draft.ville);
        if (found) {
          this.form.patchValue({ localisationId: found.id }); // ✅
        }
 ...
 this.form.valueChanges
  .pipe(debounceTime(0), takeUntil(this.destroy$))  // ← debounce 0 pour réactivité immédiate
  .subscribe(v => {
    this.formValue.set(this.form.getRawValue()); // ✅ met à jour le signal
    // Sauvegarde draft avec délai séparé
  });
...
onVilleChange(event?: Event): void {
  // Lire directement depuis l'event — plus fiable que form.value.ville
  ...
  if (villeNom) {
    ...
    if (found) {
      this.form.patchValue({ localisationId: found.id }); // ✅ ID réel
    }
  ...
  private loadQuartiers(ville: string): void {
    this.locApi.getQuartiers(ville).subscribe(
      (r) => this.allQuartiers.set(r.data ?? []), // string[] directement
    );
  }

  onQuartierInput(): void {
    // `quartier` est libre (string) => pas de reset d'un id de quartier.
  ...
  // addNewQuartier(): void {
  //   const nom = (this.form.value.quartierSearch ?? '').trim();
  //   const ville = (this.form.value.ville ?? '').trim();
  //   if (!nom || !ville || nom.length < 2) return;
  //   this.locApi.creerQuartier({ ville, quartier: nom }).subscribe({
  //     next: (r) => {
  //       this.allQuartiers.update((list) => [...list, r.data]);
  //       this.selectQuartier(r.data);
  //       this.toast.success(`Quartier « ${nom} » ajouté`);
  //     },
  //     error: (err) => {
  //       console.error('Erreur ajout quartier:', err);
  //       this.toast.error("Impossible d'ajouter ce quartier");
  //     },
  //   });
  // }

  selectType(id: number): void {
    this.form.patchValue({ typeBienId: id });
  }

  /* ── Icônes SVG par type de bien ── */
  ...
  private addFiles(files: File[]): void {
    ...
    if (remaining <= 0) {
      this.toast.info('Maximum 4 photos');
      return;
    }
    const valid = files
      .filter((f) => {
        if (!['image/jpeg', 'image/png', 'image/webp'].includes(f.type)) {
          this.toast.error(`${f.name} : format non supporté (JPG, PNG ou WebP)`);
          return false;
        }
        if (f.size > 4 * 1024 * 1024) {
          this.toast.error(`${f.name} : trop lourd (max 4 Mo)`);
          return false;
        }
        return true;
      })
  ...
  /* ── Brouillon ── */
  hasDraft(): boolean {
    return this.storage.hasDraft();
  }
  clearDraft(): void {
    this.storage.clearDraft();
    this.form.reset();
    this.photos.set([]);
    this.toast.info('Brouillon effacé');
  }
  ...
  onSubmit(): void {
    ...
    const v = this.form.getRawValue();
     console.log('FORM RAW VALUE:', v);          // ← voir localisationId
  console.log('villesAvecId:', this.villesAvecId()); // ← voir si chargé
...
const whatsApp = digits.startsWith('237') ? `+${digits}` : `+237${digits}`;
    ...
    this.annonceApi
      .publier({
        ...
        quartier: (v.quartier ?? '').trim(), // ✅ Ajouter le quartier saisi
      ...
      .subscribe({
        ...
        error: (err) => {
          this.submitting.set(false);
          this.submitError.set(err?.error?.message ?? 'Une erreur est survenue. Réessayez.');
        },
      });
  }
}
